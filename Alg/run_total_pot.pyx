...
from Alg.potential import compute_coarse_potential, compute_fine_potential
from Alg.dynamics import update_particles_verlet, rescale_velocities, compute_total_kinetic_energy
...
def run_simulation(
    ...
    - if step is a multiple of 5 compute coarse potential (PM main method)
    - else compute fine potential 
    - apply dynamics 
    ...
    """
    cdef int step
    cdef np.ndarray[np.float64_t, ndim=3] coarse_potential, fine_potential
    cdef np.ndarray[np.int32_t, ndim=1] particle_cell_map, cell_start
    cdef list all_data = []

    # Particle-to-cell mapping
    particle_cell_map, cell_start = construct_linked_list(x, y, z, cell_size, grid_size)

    # Compute slab boundaries for this rank 
    ...
    for step in range(num_steps):
        ...
        # Periodically repartition particles
        if step % 10 == 0:
        ...
        # Initialize positions on the first step
        if step == 0:
            x_prev = x - vx * dt
            y_prev = y - vy * dt
            z_prev = z - vz * dt
            ...
            y_prev %= box_size
            z_prev %= box_size
            KE_initial = compute_total_kinetic_energy(vx, vy, vz, mass)
            KE_threshold = 1.5 * KE_initial 
        
        if step % 5 == 0:
            KE_current = compute_total_kinetic_energy(vx, vy, vz, mass)
            if KE_current > KE_threshold:
                rescale_velocities(vx, vy, vz, mass, KE_threshold)

        # Coarse potential and update every 5 steps
        if step % 5 == 0:
            coarse_potential = compute_coarse_potential(global_density_fine, box_size, G)
            update_particles_verlet(
                x, y, z, x_prev, y_prev, z_prev, vx, vy, vz,
                coarse_potential, dt, box_size, coarse_cell_size,
                local_nz_start, local_nz_end
            )
        else:
            fine_potential = compute_fine_potential(
                x, y, z, mass, particle_cell_map, cell_start,
                h, softening_length, fine_cell_size, grid_size,
                box_size, len(cell_start)
            )
            update_particles_verlet(
                x, y, z, x_prev, y_prev, z_prev, vx, vy, vz,
                fine_potential, dt, box_size, fine_cell_size,
                local_nz_start, local_nz_end
            )

        rescale_velocities(vx, vy, vz, mass, ke_target=2.396252e30) ## Modify this depending on which main is used
    ...
    if rank == 0:
        with open(output_file, "w") as csvfile:
            csvfile.write("timestep,particle_id,x,y,z,vx,vy,vz,mass,kinetic_energy\n")
            for step, cx, cy, cz, cvx, cvy, cvz, cmass in all_data:
                KE_current = compute_total_kinetic_energy(cvx, cvy, cvz, cmass)
                for i in range(cx.shape[0]):
                    csvfile.write(f"{step},{i},{cx[i]},{cy[i]},{cz[i]},{cvx[i]},{cvy[i]},{cvz[i]},{cmass[i]},{KE_current}\n")
